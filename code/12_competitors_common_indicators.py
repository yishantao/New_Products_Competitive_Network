...
def eleven():
    ...
    ipc_data = pd.read_excel(io='../data/生成数据/08竞争对手/IPCMGSG/统计信息/02三年期IPCMG网络指标信息.xlsx')
...
def twelve():
    ...
    ipc_data = pd.read_excel(io='../data/生成数据/08竞争对手/IPCMGSG/统计信息/04五年期IPCMG网络指标信息.xlsx')
...
def thirteen():
    ...
    ipc_data = pd.read_excel(io='../data/生成数据/08竞争对手/IPCMGSG/统计信息/06三年期IPCSG网络指标信息.xlsx')
...
def fourteen():
    ...
    ipc_data = pd.read_excel(io='../data/生成数据/08竞争对手/IPCMGSG/统计信息/08五年期IPCSG网络指标信息.xlsx')
...
if __name__ == "__main__":
    ...
    p = Pool(4)
    p.apply_async(eleven)
    p.apply_async(twelve)
    p.apply_async(thirteen)
    p.apply_async(fourteen)
    p.close()
    p.join()
