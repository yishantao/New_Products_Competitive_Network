...
def enterprise_symbol_mapping():
    """构建企业名称与编码对应映射"""
    df = pd.read_excel(io='../data/生成数据/01企业名称修改表/13整车数据ZZCMCZ编码表.xlsx',
                       usecols=[0, 1],
...
def patent_to_symbol():
    ...
    automobile.to_excel(excel_writer='../data/生成数据/02整车数据/04整车数据(企业名称修改为ZZCMC编号).xlsx',
                        index=None)
...
def export_three_matrix_file():
    """导出三年期竞争关系矩阵"""
    automobile = pd.read_excel(io='../data/生成数据/02整车数据/04整车数据(企业名称修改为ZZCMC编号).xlsx',
