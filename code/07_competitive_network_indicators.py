...
def calculate_networks_indicators(graph, year, indicator_type):
    ...
    basic_information(graph, nodes, year, indicator_type)
    # p = Pool(4)
    ...
    network_indicators = network_indicators[
        ['nodes', 'degree_centrality', 'betweenness_centrality', 'local_reaching_centrality', 'triangles', 'clustering',
         'tie_strength']]
...
def network_three_indicators():
    ...
    for i in range(1985, 2015):
        p.apply_async(network_three_function, (i,))
    p.close()
    p.join()
...
def network_five_indicators():
    ...
    p = Pool(4)
    for i in range(1985, 2015):
        p.apply_async(network_five_function, (i,))
    p.close()
    p.join()
...
def construct_panel_data():
    ...
    total_network_indicators = total_network_indicators[
        ['OGNM', 'YEAR', 'cpntdgr3yw', 'cpntbtw3yw', 'cpntlcrch3yw', 'cpnttrgl3yw', 'cpntclust3yw', 'cpnttiestr3yw']]
    ...
    total_network_indicators = total_network_indicators[
        ['OGNM', 'YEAR', 'cpntdgr5yw', 'cpntbtw5yw', 'cpntlcrch5yw', 'cpnttrgl5yw', 'cpntclust5yw', 'cpnttiestr5yw']]
    auto_panel = pd.merge(left=auto_panel, right=total_network_indicators, on=['OGNM', 'YEAR'], how='left')
