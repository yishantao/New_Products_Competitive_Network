# -*- coding:utf-8 -*-
"""为第十一步之后做准备：计算IPCMG、IPCSG矩阵的网络指标"""
...
def calculate_enterprise_indicators_three_year_ipcmg():
    """计算企业网络指标(三年期IPCMG)"""
    institutions = pd.read_excel(io='../data/生成数据/08竞争对手/专利数据/汽车产业(企业名称).xlsx')
    ...
    for i in range(len(institutions)):
        ...
        for j in range(1985, 2013):
            ...
            file_path = '../data/生成数据/08竞争对手/IPCMGSG/三年期IPCMG/' + each_institutions + '/' + years_interval + '年IPCMG矩阵.csv'
            if os.path.exists(file_path):
                ...
                if not nx.is_empty(graph):
                    ...
                    excel_path = '../data/生成数据/08竞争对手/IPCMGSG/三年期IPCMG(网络指标)/' + each_institutions + '/' + years_interval + '年IPCMG矩阵'
    ...
    description_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/01三年期IPCMG矩阵描述信息.xlsx',
                                     index=False)
    statistical_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/02三年期IPCMG网络指标信息.xlsx',
                                     index=False)
...
def calculate_enterprise_indicators_five_year_ipcmg():
    ...
    for i in range(len(institutions)):
        ...
        for j in range(1985, 2011):
            ...
            file_path = '../data/生成数据/08竞争对手/IPCMGSG/五年期IPCMG/' + each_institutions + '/' + years_interval + '年IPCMG矩阵.csv'
            if os.path.exists(file_path):
                graph = create_diagrams(file_path)
                if not nx.is_empty(graph):
                    description_information.loc[each_institutions, years_interval] = 1
                    network_indicators = calculate_networks_indicators(graph)
                    excel_path = '../data/生成数据/08竞争对手/IPCMGSG/五年期IPCMG(网络指标)/' + each_institutions + '/' + years_interval + '年IPCMG矩阵'
                    folder = os.path.exists(excel_path)
                    if not folder:
                        os.makedirs(excel_path)
                    network_indicators.to_excel(excel_writer=excel_path + '/相关指标.xlsx',
    ...
    description_information = description_information.reset_index(drop=False)
    description_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/03五年期IPCMG矩阵描述信息.xlsx',
                                     index=False)
    statistical_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/04五年期IPCMG网络指标信息.xlsx',
                                     index=False)
...
def calculate_enterprise_indicators_three_year_ipcsg():
    ...
    for i in range(len(institutions)):
        ...
        for j in range(1985, 2013):
            years_interval = str(j) + '-' + str(j + 2)
            file_path = '../data/生成数据/08竞争对手/IPCMGSG/三年期IPCSG/' + each_institutions + '/' + years_interval + '年IPCSG矩阵.csv'
            if os.path.exists(file_path):
                graph = create_diagrams(file_path)
                if not nx.is_empty(graph):
                    description_information.loc[each_institutions, years_interval] = 1
                    network_indicators = calculate_networks_indicators(graph)
                    excel_path = '../data/生成数据/08竞争对手/IPCMGSG/三年期IPCSG(网络指标)/' + each_institutions + '/' + years_interval + '年IPCSG矩阵'
                    folder = os.path.exists(excel_path)
                    if not folder:
                        os.makedirs(excel_path)
                    network_indicators.to_excel(excel_writer=excel_path + '/相关指标.xlsx',
    ...
    description_information = description_information.reset_index(drop=False)
    description_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/05三年期IPCSG矩阵描述信息.xlsx',
                                     index=False)
    statistical_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/06三年期IPCSG网络指标信息.xlsx',
                                     index=False)
...
def calculate_enterprise_indicators_five_year_ipcsg():
    ...
    for i in range(len(institutions)):
        ...
        for j in range(1985, 2011):
            years_interval = str(j) + '-' + str(j + 4)
            file_path = '../data/生成数据/08竞争对手/IPCMGSG/五年期IPCSG/' + each_institutions + '/' + years_interval + '年IPCSG矩阵.csv'
            if os.path.exists(file_path):
                graph = create_diagrams(file_path)
                if not nx.is_empty(graph):
                    description_information.loc[each_institutions, years_interval] = 1
                    network_indicators = calculate_networks_indicators(graph)
                    excel_path = '../data/生成数据/08竞争对手/IPCMGSG/五年期IPCSG(网络指标)/' + each_institutions + '/' + years_interval + '年IPCSG矩阵'
                    folder = os.path.exists(excel_path)
                    if not folder:
                        os.makedirs(excel_path)
                    network_indicators.to_excel(excel_writer=excel_path + '/相关指标.xlsx',
    ...
    description_information = description_information.reset_index(drop=False)
    description_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/07五年期IPCSG矩阵描述信息.xlsx',
                                     index=False)
    statistical_information.to_excel(excel_writer='../data/生成数据/08竞争对手/IPCMGSG/统计信息/08五年期IPCSG网络指标信息.xlsx',
                                     index=False)
...
if __name__ == "__main__":
    ...
    p = Pool(4)
    p.apply_async(calculate_enterprise_indicators_three_year_ipcmg)
    p.apply_async(calculate_enterprise_indicators_five_year_ipcmg)
    p.apply_async(calculate_enterprise_indicators_three_year_ipcsg)
    p.apply_async(calculate_enterprise_indicators_five_year_ipcsg)
    p.close()
    p.join()
